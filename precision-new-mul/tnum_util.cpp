...
get_concrete_values_contained_in_tnum(tnum_t t, int bitvec_width) {
...
bool tnums_comparable(tnum_t t1, tnum_t t2, int bitvec_width) {
  std::unordered_set<u64> conc_in_t1 =
      get_concrete_values_contained_in_tnum(t1, bitvec_width);
  std::unordered_set<u64> conc_in_t2 =
      get_concrete_values_contained_in_tnum(t2, bitvec_width);
  return (is_subset_of(conc_in_t1, conc_in_t2) ||
          is_subset_of(conc_in_t2, conc_in_t1));
}

std::string tnum_to_string(tnum_t t, int bitvec_width) {
  char tnum_string[65];
  tnum_sbin(tnum_string, 65, t);
  return std::string(tnum_string).substr(64 - bitvec_width) +
         std::string(" (") + std::to_string(t.value) + std::string(":") +
